...
- [ ] T019 [US4] Complete `harness/continuity/source-aware-contract.test.ts` reference-integrity and negative self-mutations: remove F7, remove one P0 Issue entry, authorize the F6 claim, leak a denied F7 record, remove one F4 source branch, or allow raw diagnostic/export; each mutation must be rejected
- [ ] T020 [US4] Run focused test, schema-freeze test, and harness typecheck to green; regenerate `harness/contract-hashes.json` only after all three versioned JSON artifacts are stable
- [ ] T021 [US4] Commit the US4 contract slice with sign-off: `git commit -s -m "feat(contract): freeze source-aware sharing"`
