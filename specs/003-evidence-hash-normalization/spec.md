...
- **FR-003b**: `real-cli-e2e` への昇格根拠として用いる証拠の名指しは、その観測を生んだ run の素性（CLI 種別と正確な版、隔離設定、記録の失敗有無、終了状態）を、fixture の自己申告ではなく取得側が生成した記録によって示さなければならない（MUST）。この記録を伴わない名指しは棄却せず、`real-cli-e2e` の根拠にしないだけとする（FR-006d / FR-017a）
