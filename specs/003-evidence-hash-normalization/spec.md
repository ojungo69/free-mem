...
同時に、16 件の観測記録は本物なので、この作業で初めて `real-cli-e2e` を**正当に**名乗れる cell が
生まれる。移行は「降格を避ける後始末」ではなく「証拠に裏付けられた最初の昇格」になる。
...
3. **Given** 観測記録が実在し digest も一致する、**When** matrix を組み立てる、**Then** その cell は
   `real-cli-e2e` として記録される
...
- **FR-003b**: 実 CLI 観測を根拠とする fixture は、その観測を生んだ run の素性（CLI 種別と正確な版、隔離設定、記録の失敗有無、終了状態）を、fixture の自己申告ではなく取得側が生成した記録によって示さなければならない（MUST）
...
- **FR-006b**: fixture が挙げた根拠 event の名前は、名指しした観測記録に実在しなければならない（MUST）。実在しない event を根拠に挙げた主張は棄却しなければならない（MUST）
...
- **FR-010**: 正規化は、観測に現れた欄の有無と並びを保存しなければならない（MUST）。値を伏せる場合も欄そのものを落としてはならない（MUST NOT）
...
- **FR-017**: 既存の観測記録 16 件に対応する既存 fixture は、その記録から算出した digest を結び付けることで `real-cli-e2e` へ昇格しなければならない（MUST）。対応する観測記録を持たない cell は `source-test` のままでなければならない（MUST）。この作業で証拠強度が下がる cell があってはならない（MUST NOT）
...
- **SC-005**: 成果物・診断出力・CI ログを、既存 16 件の観測記録に現れる絶対 path・投入指示・モデル出力の文字列で検索したとき、ヒットが 0 件になる（現在は 2 件ヒットする）
...
- **SC-008**: 移行後、`real-cli-e2e` を名乗る cell のうち観測記録による再計算を通っていないものが 0 件になる。かつ移行前に `source-test` だった 21 cell のうち、証拠強度が下がったものが 0 件になる

## Assumptions

- 証拠置き場は `harness/fixtures/<cli>/raw/` とする。rig が別の場所へ書き出す場合、その成果物をこの置き場へ同一内容で持ち込む工程が必要になる
- scenario の識別子は既存 fixture の `scenario` 欄を用いる。scenario 定義そのものを別ファイルへ切り出す作業は本仕様の範囲外とし、識別子の追跡のみを扱う
- `official-doc` / `source-test` を根拠とする cell には証拠 digest を要求しない。本仕様が扱うのは実 CLI 観測を名乗る cell のみ
- 既存 16 件の観測記録は隔離 rig 下で実際に取得されたものであり、真正なものとして扱う。移行はこの前提の上に成り立つ
- 現在 `real-cli-e2e` を名乗っている cell は 0 件なので、この作業で既存成果物の証拠強度が下がることは無い。証拠に裏付けられた昇格が初めて発生する側の変更になる
