...
- [ ] T002 [P] [US4] Add failing byte/control/canonical-URL ProviderProposal/Choice, local-HTTP credential/restricted rejection, local-HTTPS peer, protocol/resource/fingerprint/successor/job-bound signal cases plus closed provider-egress receipt with explicit non-prefix committed IDs/count/fingerprint, non-overlapping plateau action/sample time, CA/six raw-TLS-receipt, pair-bound conflict, and exact-16+1 mutations to `specs/005-product-reset/contracts/validate-slice1-fixture.test.mjs`, `specs/005-product-reset/contracts/validate-alpha-result.test.mjs`, `specs/005-product-reset/contracts/validate-alpha-result-failure.test.mjs`, `specs/005-product-reset/contracts/validate-alpha-result-input.test.mjs`, and `specs/005-product-reset/contracts/validate-alpha-runner-evidence.test.mjs`
...
- [ ] T035 [US1] Compile local HTTP to `providerPeerTrust=unverified` and credential-none/eligible-only, set `verified` only after local-HTTPS peer verification, pass that value into DestinationBoundary without caller override, partition trusted local events by exact verified repository before projection, reject mixed/unknown groups content-free, project before context/transcript/prompt/request creation, invoke the atomic privacy skip for all-restricted jobs, validate bounded exact citations and the active-attempt output limit, and persist strongest sensitivity/repository plus lineage/revision/manifest/provider/attempt provenance in `vendor/codemem/packages/core/src/raw-event-flush.ts`, `vendor/codemem/packages/core/src/ingest-types.ts`, `vendor/codemem/packages/core/src/ingest-pipeline.ts`, `vendor/codemem/packages/core/src/ingest-prompts.ts`, and `vendor/codemem/packages/core/src/ingest-xml-parser.ts`
...
- [ ] T055 Add failing pin/candidate/evidence-root/incomplete-lifecycle/fabricated-success/normal-proposal/network-deny, hostile second-UID loopback port squat, forged local-destination, candidate evidence-root discovery/access through env/argv/proc/hardlink/read/write or a self-authored bundle, same-payload sensitivity/quarantine escalation with late-derived cascade, pair-bound same-ID/different-digest receipt reuse/distinct-conflict, missing/mutated provider-egress authorization/time/count/sensitivity receipt, runner-stub actual-byte versus policy/candidate sensitivity-count forgery, plateau workload/drain/checkpoint/sample order/overlap/receipt/no-op-delta, CA/six raw-TLS-receipt/trust-anchor/peer-cert/SNI/timing/zero-byte/item/token/concurrency, early-drain ACK/post-drain work, and exact-16+1 suite self-tests in `harness/slice1-runtime.test.mjs`
...
- [ ] T057 Populate the corrected runner bundle with runner-owned provider-egress observations carrying explicit canonical-order committed event IDs/count/fingerprint, network/CA/six raw TLS trust receipts, and 12 raw workload/drain/checkpoint/resource windows with strict non-overlapping action/sample timestamps; bind them through result fingerprints/aggregates and add attempted/final render, manifest/provider/job/retry/lifecycle/pair-bound-conflict/16+1/p95 evidence in `harness/slice1-runtime.mjs`
