...
- [ ] T049 [P] install ownership manifest（判断 #10、roundtrip 検証込み）。前提: T034
- [ ] T050 pre-migration backup 機構: `db.backup()` 実測（不在なら §20.2 ADR 後実装）→ online backup + verify を migration（判断 #15）・破壊的操作（T046/T047）の precondition に配線。失敗 = 破壊操作開始しない（HI-25）。前提: T034, T035
