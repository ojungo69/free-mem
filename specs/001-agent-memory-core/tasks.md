...
- [X] T056 Exit-3 no Agent blockage: daemon 不在/認証 fail/handshake 不適合/disk full/size 超過/hung daemon/partial response/backpressure/長時間 lock holder/catastrophic regex で hook fail-open + Agent 継続。exit code + 最大実時間（Agent 別 deadline + spool 予約発動）の両 assert。実 hook timeout 直前保全。p95 記録 + doctor 表示
  ...
  - 検証証拠: `harness/phase1-no-agent-blockage.mjs`、`evidence/phase1-t056-no-agent-blockage-validation.md`、13 files / 314 tests、packed artifact、lint、static scan 279 files / 0 violations、artifact SHA-256一致。T058 candidate 再実行の最終 p95 は Claude 148.3ms / Codex 139.3ms で 150ms 目標内（過去の nonblocking warning は evidence に保持）
...
- [X] T058 SC-1 candidate 照合 + clean checkout gate（cwd = 一時 worktree の `vendor/codemem`、exact node v24.16.0 / corepack pnpm 11.8.0）: frozen-lockfile install → build → check + CLI help + viewer smoke + test 集合機械比較（`4,037 - 2,376 + 193 = 1,854`、CI check `1,851 passed + 3 todo`）+ §32.12 upgrade/rollback コマンド記録 → speckit-verify-tasks（machine 25/31 を維持し manual 31/31 と分離）→ correctness / security review gate（妥当 finding を修正、daemon-job は Class-C 契約により reject）→ ponytail-review → T029/T053 static scan 照合。product candidate = `3da6826`、live Sonar new-code coverage `81.551%`（`61acb3d` 解析、final push で再確認）。**`main` merge は候補検証の外にある最終 external step で、未実行。** 実行根拠: `evidence/phase1-t058-final-validation.md`
