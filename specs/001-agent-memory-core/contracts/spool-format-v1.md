...
| `spoolDir` (`rootDir`) | `<controlDir>/spool` | `packages/core/src/storage-layout.ts:70`, `packages/core/src/spool.ts:172` |
| `tmpDir` | `<spoolDir>/tmp` | `packages/core/src/spool.ts:175` |
| `readyDir` | `<spoolDir>/ready` | `packages/core/src/spool.ts:176` |
| `quarantineDir` | `<spoolDir>/quarantine` | `packages/core/src/spool.ts:177` |
| `lockPath` | `<spoolDir>/lock` | `packages/core/src/spool.ts:178` |
| `counterPath` | `<spoolDir>/dropped-counter` | `packages/core/src/spool.ts:179` |

The daemon's own writer lock (`<controlDir>/lock.db`, a SQLite `BEGIN IMMEDIATE` file lock) is a **separate** file from the spool lock (`packages/core/src/storage-layout.ts:67` vs. `packages/core/src/spool.ts:178`, `packages/core/src/daemon-lifecycle.ts:164-181`). A reimplementation must not conflate the two.

### 1.1 Directory permission enforcement

`ensureSpoolDirectories()` calls `ensurePrivateDirectory()` on `rootDir`, `tmpDir`, `readyDir`, and `quarantineDir` every time the spool is touched (`packages/core/src/spool.ts:183-188`), and it is invoked on **every** lock acquisition (`acquireSpoolLock` → `ensureSpoolDirectories`, `packages/core/src/spool.ts:285-286`), i.e. on every `spoolMutation`, every sweep, `readSpoolStatus`, and `quarantineSpoolEntry` call.
...
Individual spool files are created `0o600`: the lock owner file via `openSync(lockPath, "wx", 0o600)` (`spool.ts:298`), the drop counter via `writeFileSync(..., { mode: 0o600 })` (`spool.ts:379-383`), and ready/tmp entry files via `writeFileSync(..., { mode: 0o600 })` (`spool.ts:907-912`).
...
|---|---|---|
| `quotaClass` | `"normal"` or `"reserved"` (§5) | `spool.ts:770-771` |
| key hash | `sha256(idempotencyKey, "utf8").hex()` | `spool.ts:770,849` |
| `payloadHash` | `sha256(canonicalMutationJson({ method, body, redaction }), "utf8").hex()` | `spool.ts:644,767`; `packages/core/src/mutation-dispatcher.ts:65-67` |

- Ready file: `<stem>.json` in `readyDir`.
- Temp file: `<stem>.json.tmp` in `tmpDir` — i.e. exactly the ready file name with `.tmp` appended (`spool.ts:772,852`). `parseSpoolEntry` enforces this exact relationship when validating a file against its own claimed name (`spool.ts:772-774`).
- Quarantine file: `<reason>-<original-name>` placed flat in `quarantineDir`, where `reason` is `"broken_json"` or `"idempotency_conflict"` and `<original-name>` is the source file's own basename (`spool.ts:939,950`). There are no quarantine subdirectories.
...
1. It checks whether the content-addressed `readyPath` already exists (`spool.ts:857-874`).
2. If it exists and its on-disk content is byte-for-byte identical to the freshly canonicalized entry, `spoolMutation` performs two `fsync`s (`tmpDir` then `readyDir`) and returns `{status: "duplicate", quotaClass, path: readyPath}` **without writing anything** — the existing file is left untouched (`spool.ts:867-870`).
3. If it exists but its content differs from the freshly canonicalized entry (a content-hash collision — normally unreachable given content-addressed naming, but defensively handled), this is treated as an I/O failure via `resultForIoFailure` (`spool.ts:858-865`); see §7.2 for what this does to the drop counter.
4. Only if no matching `readyPath` exists does `spoolMutation` check whether a matching `tmpPath` (the same stem with `.tmp` appended) already exists (`spool.ts:875-893`).
5. If `tmpPath` exists and its content matches, this is an interrupted prior write; `spoolMutation` finishes it — `fsyncPath(tmpPath)`, `renameSync(tmpPath, readyPath)`, `fsyncPath` of both directories — and returns `{status: "queued", quotaClass, path: readyPath}`, **not** `{status: "duplicate", ...}` (`spool.ts:885-890`). This is the "rejected as already-in-progress" case being finished, not rejected.
6. If `tmpPath` exists but its content differs, this is likewise treated as an I/O failure (`spool.ts:876-883`).
...
The file's exact byte content is `canonicalMutationJson(entry) + "\n"` (`spool.ts:776,853`). On read, `parseSpoolEntry` re-serializes the parsed object and requires the stored bytes to equal that canonical form exactly, or the entry is treated as unparseable and quarantined (`spool.ts:776-778`).
...
```
Evidence: `spool.ts:124-132` (type), `spool.ts:54-62` (allowed field set `SPOOL_ENTRY_FIELDS`), `spool.ts:731-739` (parse-time validation). Any field not in `{version, method, idempotencyKey, payloadHash, quotaClass, redaction, body}` makes the entry invalid (`spool.ts:506-513,730`). `version` must be exactly `1`. `payloadHash` must match `/^[a-f0-9]{64}$/` (`spool.ts:51,735-736`).
...
```
Evidence: `spool.ts:134-140` (type), `spool.ts:63-69` (allowed field set `REDACTION_FIELDS`), `spool.ts:705-720` (validator `validateSpoolRedaction`).

- `secret_rules_version` must match `^[a-f0-9]{64}(:degraded)?(\+[a-f0-9]{64}(:degraded)?)*$` (`spool.ts:52`, applied `spool.ts:711`).
- Invariant enforced on every parse: `redaction_degraded === secret_rules_version.includes(":degraded")` (`spool.ts:715`). A mismatch is treated as malformed.

### 3.4 Body fields per method

| `method` | Allowed body fields | Evidence |
|---|---|---|
| `POST /v1/events` | `idempotencyKey`, `event` | `spool.ts:71` |
| `POST /v1/memories/record` | `idempotencyKey`, `kind`, `title`, `body`, `confidence`, `project` | `spool.ts:72` |

Unknown body fields are rejected (`rejectUnknownFields`, `spool.ts:506-513`, invoked at `spool.ts:585,746`).

`body.idempotencyKey` must equal the envelope's `idempotencyKey` (`spool.ts:586-588,747-749`).

For `POST /v1/events`, `body.event` must be a normalized event object (validated by `validateNormalizedEvent`, whose required-field set is `NORMALIZED_EVENT_FIELDS` in `packages/core/src/normalized-event.ts:9-33`); `event.idempotencyKey` must equal the envelope key, and `event.sensitivity` must equal `redaction.sensitivity` (`spool.ts:611-614,751-757`). The event's `quotaClass` consistency is checked as described in §5.

For `POST /v1/memories/record`, the body must satisfy `validatePreparedMemoryBody` (`spool.ts:515-539`): `kind` passes `validateMemoryKind`; `title` and `body` are non-empty strings; `project`, if present, is a string; `confidence`, if present, is a finite number in `[0, 1]`.

When the computed sensitivity is `secret`, or the redaction is `redaction_degraded`, a prepared memory-record body has `title`/`body` overwritten with a placeholder (`"[REDACTED:secret]"` or `"[REDACTED:degraded]"`) and `project` deleted, **before** the body is hashed and written (`spool.ts:633-640`). The event branch is **not** symmetric with this: `payload = {}` via `sealDegradedNormalizedEvent` is applied only when this write's redaction is degraded or a merged-in previous redaction was degraded (`redacted.degraded || previousRedaction?.redaction_degraded`). A non-degraded event whose computed sensitivity is `secret` keeps its redacted payload and only has `sensitivity` forced to `"secret"` — it is **not** sealed (`spool.ts:606-615`; the same sealing happens upstream in the RPC clients before the fallback spool write, `packages/cli/src/commands/hook-rpc-client.ts:264-270`, `packages/mcp-server/src/rpc-client.ts:218-224`).
...
Independent of the entry envelope, the raw `idempotencyKey` string itself must be (`validateIdempotencyKey`, `spool.ts:483-504`):
- a `string`,
- non-empty,
- ≤ 256 UTF-8 bytes (`Buffer.byteLength(value, "utf8") <= 256`),
- free of C0 control characters and DEL (any code point `< 0x20` or `=== 0x7f`),
- unchanged after being passed through the redaction preprocessor with `allowlist: ["idempotencyKey"]` — i.e. it must not itself look like sensitive content, or the mutation is rejected (`spool.ts:498-503`).
...
- Once a mutation reaches `spoolMutation`, the key is validated to equal `body.idempotencyKey` and, for events, `event.idempotencyKey` (`spool.ts:586-588,611-613`); it is never rewritten by the spool layer itself.
- On a spool-fallback write that follows a failed RPC, the caller may pass `previousRedaction` (the redaction metadata already computed for the RPC attempt). `prepareMutation` then merges it with the freshly recomputed redaction via `mergeSpoolRedaction` (monotonic union: `secret` beats `private` beats `normal`; booleans OR together; `secret_rules_version` sets are unioned and sorted) rather than re-running user redaction rules from scratch, specifically to avoid a second run mutating required IDs on an already-degraded payload (`spool.ts:551-575,593-595`). This does not change the `idempotencyKey`, only the `redaction` field baked into `payloadHash`.
...
|---|---|---|
| `SPOOL_NORMAL_QUOTA_BYTES` | 128 MiB (134,217,728) | `spool.ts:35` |
| `SPOOL_RESERVED_QUOTA_BYTES` | 16 MiB (16,777,216) | `spool.ts:36` |
| `SPOOL_QUARANTINE_QUOTA_BYTES` | 32 MiB (33,554,432) | `spool.ts:37` |
| `SPOOL_FILE_MAX_BYTES` | 64 KiB (65,536) | `spool.ts:38` |
| `SPOOL_RESERVED_MIN_EVENTS` | 64 | `spool.ts:39` (see note below) |

- `quotaClass` is `"reserved"` **only** for `POST /v1/events` whose normalized `event.kind` is in `RESERVED_EVENT_KINDS = {"pre_compact", "session_ended"}` (`spool.ts:50,623`). Every other event kind, and **every** memory record regardless of kind, is `quotaClass: "normal"` (`spool.ts:592,623`). This is re-validated on parse: for events, `quotaClass !== (RESERVED_EVENT_KINDS.has(kind) ? "reserved" : "normal")` is an error (`spool.ts:754-759`); for memory records, `quotaClass !== "normal"` is an error (`spool.ts:761-764`).
- Usage accounting is **not** a persisted counter. `scanUsage()` walks `tmpDir` and `readyDir` fresh, under the spool lock, on every quota check, classifying each file by whether its name starts with `"reserved-"` (`scanDirectory`, `spool.ts:656-673`, invoked by `scanUsage`, `spool.ts:675-688`, and by `spoolMutation`'s admission check, `spool.ts:896-899`).
...
- Quarantine usage is tracked separately from normal/reserved quota (`scanDirectory` with `area === "quarantine"` only increments `usage.quarantineBytes`/`usage.quarantineFiles`, `spool.ts:663-666`).
- Admission: for a candidate entry of size `bytes`, let `used` be the scanned bytes already in that entry's `quotaClass` bucket and `quota` be that class's byte limit. If `used + bytes > quota`, the write is dropped with `reason: "quota_full"` and the drop counter is incremented (`spool.ts:896-904`).
...
1. `scanDirectory` (`spool.ts:656-673`) — the function underlying every quota-usage scan (`scanUsage`, itself invoked by `spoolMutation`'s admission check at `spool.ts:896`, by `moveToQuarantineLocked`'s quarantine-quota check at `spool.ts:945`, and by `readSpoolStatus` at `spool.ts:1081`) — `lstatSync`s **every** entry in whichever directory it scans (`tmp/`, `ready/`, or `quarantine/`) and throws `"Spool directories may contain only regular files."` if any single entry is not a regular, non-symlink file (`spool.ts:659-662`), regardless of that entry's filename or whether it is the file the caller actually cares about.
...
3. The throw is not caught locally by `scanUsage`. `readSpoolStatus` (`spool.ts:1076-1106`) has no `try/catch` around its `scanUsage` call, only a `finally` releasing the lock, so the throw propagates to `readSpoolStatus`'s own caller.
4. Inside `spoolMutation`, the same throw during admission (`spool.ts:896`) is caught by the function's own outer `try/catch` (`spool.ts:924-925`) and reported via `resultForIoFailure` as `{status: "dropped", reason: "io_error"}`; because this occurs after the spool lock is held, the drop counter **is** incremented for it (§7.2). This recurs for every subsequent write attempt sharing the same `quotaClass` bucket until the offending non-regular entry is removed from the spool directories.
5. Inside `importReadySpoolEntries`, the same throw from a `scanUsage` call nested inside `moveToQuarantineLocked` behaves differently depending on where it originates: if raised during `recoverTmpEntriesLocked` (run once per `importReadySpoolEntries` call, before the per-entry loop), it aborts the entire tmp-recovery pass and is caught by the outer `try/catch` at `spool.ts:996-1003`, logged as "spool importer recovery failed; entries were retained." — not a per-entry failure. If raised later, from a `moveToQuarantineLocked` call inside the per-ready-entry loop (`spool.ts:1021` or `spool.ts:1039`), it is caught by that entry's own `try/catch` (`spool.ts:1014,1046-1047`) and logged as "spool importer could not process an entry; it was retained.", leaving only that one ready file in place for the next sweep.
...
- `WARNING_RATIO = 0.8` (`spool.ts:44`).
- **Write-time**: immediately after a successful admission and write, if `(used + bytes) / quota >= 0.8` for the entry's own `quotaClass`, `spoolMutation` emits a single warning through the `onWarning` callback (default `console.error`) with the message `` `${quotaClass} spool usage reached 80%.` `` (`spool.ts:920-922`, `warn()` helper `spool.ts:446-452`). This is edge-observed per call, not edge-triggered/deduplicated across calls — a caller that keeps writing near the threshold will see the warning repeatedly.
- **Status-time**: `readSpoolStatus()` independently recomputes usage and returns a `warnings: string[]` array containing `"normal spool usage reached 80%"`, `"reserved spool usage reached 80%"`, and/or `"spool quarantine usage reached 80%"` whenever the corresponding ratio is `>= 0.8` at call time (`spool.ts:1076-1106`). There is no persisted "already warned" state; this is a pure recomputation.
- `critical: true` iff any of: `dropped.total > 0`, `dropped.quarantineRejected > 0`, `usage.reservedBytes >= SPOOL_RESERVED_QUOTA_BYTES`, or `usage.quarantineBytes >= SPOOL_QUARANTINE_QUOTA_BYTES` (`spool.ts:1097-1101`). Note that `normalBytes` reaching 100% of quota is **not** itself a `critical` condition — it only becomes visible via `dropped.total` once an actual normal-class write is subsequently refused.
...
```
Evidence: `spool.ts:115-122` (type), `spool.ts:43` (timestamp regex `COUNTER_TIMESTAMP`), `spool.ts:387-420` (`readCounter` validation — `byKind` keys must each be `"memory_record"` or a recognized normalized-event kind, `spool.ts:410-418`).

The file is a **fixed-size, space-padded 4096-byte record** (`COUNTER_BYTES = 4096`, `spool.ts:42`): `encodeCounter()` serializes the JSON, throws if it exceeds 4096 bytes, and pads the remainder with ASCII spaces (`0x20`) (`spool.ts:369-375`). It is created lazily with `flag: "wx"` (fails if it already exists) the first time it's needed (`initializeCounter`, `spool.ts:377-385`), and thereafter is rewritten **in place** by opening with `"r+"` and overwriting the full 4096-byte buffer, `fsyncSync`, close — not via a temp-file-and-rename durable replace (`writeCounter`, `spool.ts:422-436`).

### 7.2 What increments it, and what does not

`incrementDrop(layout, kind, onWarning)` (`spool.ts:462-481`) bumps `total`, `byKind[kind]`, sets `firstDroppedAt` if it was `null`, and always updates `lastDroppedAt` to the current ISO timestamp.

| Drop `reason` | Increments drop counter? | Where |
|---|---|---|
| `"quota_full"` | Yes | `spool.ts:901` |
| `"disk_full"` / `"io_error"` from a write/rename/duplicate-mismatch failure **after the spool lock is held** | Yes | `resultForIoFailure` → `incrementDrop`, `spool.ts:795-809`, called from `spool.ts:859-864,871-873,876-881,891-893,916-918,924-925` |
| `"invalid"` (mutation failed `prepareMutation`) | **No** | `spool.ts:811-823` — this branch returns before `layout` is even resolved, so `incrementDrop` is never called |
| `"lock_timeout"` | **No** | `spool.ts:826-833` |
| `"disk_full"` / `"io_error"` encountered while **acquiring** the spool lock (before the lock is held) | **No** | `spool.ts:834-844` |

`byKind`'s key is `"memory_record"` for `POST /v1/memories/record`, or the (best-effort) `String(event.kind)` — falling back to `"unknown"` if it can't be read — for `POST /v1/events` (`dropKind`, `spool.ts:454-460`).

If `incrementDrop` itself throws (e.g. the disk is so full that even the fixed 4096-byte counter rewrite fails), the error is swallowed and only a warning is emitted; the drop is **not** retried or recorded, so `total` under-counts in that corner case (`spool.ts:477-480`, comment: "A completely full disk may prevent even the preallocated counter rewrite.").

### 7.3 Quarantine-rejection counter

`quarantineRejected` is incremented only via `incrementQuarantineRejected()` (`spool.ts:438-444`), called exactly when `moveToQuarantineLocked` finds that moving an entry into quarantine would push `quarantineBytes` over `SPOOL_QUARANTINE_QUOTA_BYTES` (`spool.ts:945-949`). In that case the source entry is **left in place** (not deleted, not moved) and the function returns `{ status: "full" }` instead of moving it.

---

## 8. Quarantine and conflict rules

`moveToQuarantineLocked(layout, area, name, reason)` (`spool.ts:933-956`), called only while the spool lock is held:

1. `basename(name) !== name` is rejected (path traversal guard) (`spool.ts:939`).
2. The source (`tmp/<name>` or `ready/<name>`) must be a regular, non-symlink file (`spool.ts:941-944`).
...
4. Otherwise, `renameSync` to `quarantine/<reason>-<name>`. If that destination already exists, `moveToQuarantineLocked` **throws** (`spool.ts:951`) — see [Known gaps](#known-gaps-documented-not-fixed).
5. On success, both the source directory and the quarantine directory are `fsync`'d (`spool.ts:952-954`).

`reason` is one of:

| Reason | Meaning | Raised from |
|---|---|---|
| `"broken_json"` | Entry failed to parse/validate as a well-formed `SpoolEntry`, or a recovered `tmp` entry no longer matches its already-promoted `ready` counterpart | `recoverTmpEntriesLocked` (`spool.ts:958-981`), `importReadySpoolEntries` on a parse failure (`spool.ts:1017-1026`) |
| `"idempotency_conflict"` | The daemon's mutation dispatcher found an existing committed mutation under the same `(method, idempotencyKey)` with a **different** `payloadHash` | `dispatchSpoolMutation` returning `"conflict"` on `MutationConflictError` (`packages/core/src/daemon-rpc.ts:881-897`), handled in `importReadySpoolEntries` (`spool.ts:1038-1043`) |

`quarantineSpoolEntry(dataDir, readyName, reason)` (`spool.ts:1054-1074`) is the externally callable form (e.g. for operator tooling): it requires `readyName` to be a bare basename ending in `.json` and `reason` to be one of the two values above, acquires the spool lock itself, and delegates to `moveToQuarantineLocked` against `readyDir`.
...
The spool lock is a plain file at `spool/lock` (not a `flock`/`fcntl`-style advisory lock) containing a JSON owner record, created with `O_CREAT|O_EXCL` semantics (`openSync(path, "wx", 0o600)`, `spool.ts:298`):
...
```
Evidence: `spool.ts:190-196` (type), `spool.ts:287-294` (construction), `packages/core/src/storage-platform.ts:240-265` (`readProcessIdentity`, producing `startTime`/`fingerprint`).

Acquisition (`acquireSpoolLock`, `spool.ts:280-367`):

1. `ensureSpoolDirectories(layout)` (§1.1).
2. Attempt `openSync(lockPath, "wx", 0o600)`.
3. On success: write the JSON owner record (`+ "\n"`), `fsyncSync` the fd, `lstat` the path back and re-verify device/inode and owner-content identity to detect a replace-during-init race (`spool.ts:309-320`), then `fsyncPath(rootDir)` to durably record directory entry creation.
...
5. `LOCK_WAIT_MS = 5` — the poll sleep uses a synchronous `Atomics.wait` on a dedicated `SharedArrayBuffer`-backed `Int32Array` (`spool.ts:47-48,269-271`).

`removeStaleLock(lockPath)` (`spool.ts:241-267`):

- Non-existent lock file → considered removable (returns `true`, nothing to remove).
- Existing lock that is not a regular file, or is a symlink → **throws** (`spool.ts:248-250`), it does not silently steal.
- Reads the owner record. If a well-formed owner is present and `lockOwnerAlive(owner)` is true (process not a zombie/dead **and** `/proc/<pid>` start-time+fingerprint still match the recorded values, `spool.ts:228-239`), the lock is **not** stale — returns `false`.
- If there is no parseable owner record at all, an initialization grace period applies: `Date.now() - info.mtimeMs <= LOCK_INITIALIZATION_GRACE_MS` (25 ms, `spool.ts:46,253`) — a lock file younger than 25 ms with unreadable content is *not yet* considered stale (it may still be mid-write by its creator).
- Before unlinking, re-reads device/inode and owner content and only proceeds if they are unchanged from the first read (`spool.ts:255-258`) — avoids stealing a lock that was replaced between the staleness check and the removal.
- Unlinks and `fsyncPath`s the parent directory (`spool.ts:259-266`).

Release (`SpoolLockHandle.close()`, `spool.ts:322-343`): re-verifies device/inode + owner-content match before unlinking (so a lock this handle no longer actually owns — e.g. because it was reaped as stale by someone else — is left alone), unlinks, `fsyncPath`s the parent, closes the fd. A failed release is deliberately swallowed ("recovered after this process exits").

### 9.2 Deadline

| Constant | Value | Evidence |
|---|---|---|
| `SPOOL_LOCK_DEADLINE_MS` (default) | 100 | `spool.ts:40` |
| Caller-supplied bound | must be an integer in `[1, 250]` | `normalizedDeadline`, `spool.ts:787-793` |
| `LOCK_INITIALIZATION_GRACE_MS` | 25 | `spool.ts:46` |
| `LOCK_WAIT_MS` (poll interval) | 5 | `spool.ts:47` |
| `LOCK_OWNER_MAX_BYTES` | 2048 (owner file larger than this, or a symlink, is treated as absent/unowned) | `spool.ts:45,201` |

`acquireSpoolLock` validates its `deadlineMs` argument via `normalizedDeadline` as the very first step, before any I/O (`spool.ts:284`) — a direct caller of `acquireSpoolLock` observes a synchronous throw on an out-of-range value. `spoolMutation`, however, calls `acquireSpoolLock` inside its own `try { … } catch (error) { … }` block (`spool.ts:826-844`), and that catch is a blanket catch: only `SpoolLockTimeoutError` is special-cased (`spool.ts:830-833`). A deadline-validation `Error` is not that type, so it falls through to the generic branch (`spool.ts:834-844`), and `spoolMutation` *returns* `{status: "dropped", quotaClass: entry.quotaClass, reason: "io_error"}` instead of throwing to its own caller. Because this failure occurs while *acquiring* the lock (before the lock is held), the drop counter is **not** incremented for it (§7.2). A caller passing an invalid `lockDeadlineMs` to `spoolMutation` therefore observes an ordinary dropped-write result indistinguishable from a real lock-acquisition I/O failure, not a synchronous exception. On a genuine lock-acquisition timeout (a valid deadline that simply expires), `spoolMutation` returns `{status: "dropped", reason: "lock_timeout"}`, likewise without incrementing the drop counter (§7.2).
...
- A thrown error from `importReadySpoolEntries` (including a failure to acquire the spool lock, which `importReadySpoolEntries` itself catches and logs rather than propagating, `spool.ts:990-1005`) is caught at the `sweepSpool` level and only logged; ready entries are retained for the next tick.

---

## 11. Commit-before-delete ordering

`importReadySpoolEntries(dataDir, handler)` (`spool.ts:983-1052`):
...
2. For **each** name in `names`, acquire the spool lock fresh (one acquisition per entry, so a long-running sweep does not hold the lock across an unbounded handler call):
   ...
   - **If `"committed"`**: only now is the file removed, via `durableRemoveFile(path)` (`spool.ts:1034-1036`) — `unlinkSync` followed by `fsyncPath` of the parent directory (`storage-platform.ts:95-104`).
...
### 11.1 Tmp-entry recovery (`recoverTmpEntriesLocked`, `spool.ts:958-981`)
...
1. **Drop counter under-counts certain drop reasons.** `incrementDrop` is only called for `reason: "quota_full"` and for `"disk_full"`/`"io_error"` that occur **after** the spool lock has been acquired (§7.2, `spool.ts:811-845` vs. `spool.ts:802,901`). Drops with `reason: "invalid"` (malformed mutation), `reason: "lock_timeout"`, and `reason: "disk_full"`/`"io_error"` encountered while *acquiring* the lock are never reflected in `dropped.total`, `dropped.byKind`, or the `critical` flag derived from them. This is directly observable via `readSpoolStatus()`.
...
2. **The drop counter can itself silently fail to record a drop.** If disk exhaustion prevents even the fixed 4096-byte in-place rewrite of `dropped-counter`, `incrementDrop`'s own write throws, the error is caught, only a warning is emitted, and the drop is not counted (`spool.ts:477-480`). This is an explicit, commented fail-open ("A completely full disk may prevent even the preallocated counter rewrite.") rather than an oversight.
...
4. **A duplicate-content write can be reported as `dropped` while the queued mutation is still present.** This presupposes the successful duplicate-detection path described in §2.1: when `spoolMutation` finds a ready file whose content already matches the candidate (a true duplicate submission), it still performs `fsyncPath(tmpDir)`/`fsyncPath(readyDir)` before returning `status: "duplicate"` (`spool.ts:867-870`). If that fsync itself fails, the failure is routed through `resultForIoFailure` → `incrementDrop`, returning `status: "dropped"` and bumping the drop counter (`spool.ts:871-873`) — even though the pre-existing ready file was never touched, remains on disk unchanged, and will still be picked up and committed by the next sweep. The caller is told the mutation was dropped when it is, in fact, still queued and will still be applied.
