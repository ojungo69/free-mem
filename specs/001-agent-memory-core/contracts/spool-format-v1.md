...
When the computed sensitivity is `secret`, or the redaction is `redaction_degraded`, a prepared memory-record body has `title`/`body` overwritten with a placeholder (`"[REDACTED:secret]"` or `"[REDACTED:degraded]"`) and `project` deleted, **before** the body is hashed and written (`spool.ts:633-640`). Degraded/secret events instead get `payload = {}` via `sealDegradedNormalizedEvent` (`spool.ts:606-610`; the same sealing happens upstream in the RPC clients before the fallback spool write, `packages/cli/src/commands/hook-rpc-client.ts:264-270`, `packages/mcp-server/src/rpc-client.ts:218-224`).
