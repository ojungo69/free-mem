...
| `oboete hook` | Agent-invoked entry for Claude Code, Grok Build, Codex: reads stdin JSON, detects the agent from the environment, checks the paused marker, captures after the detector runs, prints a plain-text pack on injection events | stdin JSON | pack or nothing; exit `0` |
...
| `oboete search <query> [--limit N]` | Same-repository active memories by relevance (repository from cwd) | | list with scores and reasons; empty result exit `0` with a reason |
...
| `oboete import [file\|-] [--dry-run] [--map-repo <old-id>=<current>]` | Per-line validated import (64 KB per line, 256 MB per file), derived fields recomputed, union on `content_hash`, stricter sensitivity kept, tombstones win; `--map-repo` maps a machine-local (`common_dir`) repository identity from another installation onto a repository here | | counts; `2` on invalid format |
...
Diagnostics and logs never contain credential values or redacted content.
