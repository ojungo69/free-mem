...
| `oboete hook --agent codex\|claude-or-grok` | Agent-invoked entry: reads stdin JSON, resolves `claude-or-grok` from `GROK_HOOK_EVENT`/`GROK_SESSION_ID`, checks the paused marker, reads at most 1 MB of stdin, runs the detector before any write, captures or spools, prints a plain-text pack on injection events | stdin JSON | pack or nothing; exit `0` |
| `oboete capture --agent pi --event <name>` | Pi's detached capture child; writes `spool/pi-ack/<event id>.started` first and renames it to `.done` on completion | stdin normalized event | none; exit `0` |
