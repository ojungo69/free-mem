...
| `oboete doctor [--probe-provider] [--probe-agents]` | Hook wiring by probe result and trust state (`--probe-agents` re-runs the headless wiring probes), storage integrity and FTS5, migration level, worker liveness, spool backlog and writability, provider reachability, estimated allowance and exhaustion flag, catalog paid-plan flag, unrecognized agents, native memory coexistence, config file mode, Pi diagnostics. Every item has the shape `{ item, status, reason, consequence, recovery }`: `consequence` states what the developer loses while it is degraded (for example "Codex sessions capture nothing"), `recovery` states the command or step that repairs it (for corrupt storage: back up the file, run `oboete export` if readable, move the database aside, run `oboete setup`, import the export) | | table or JSON; exit `1` when any item is degraded, `3` on storage integrity failure |
| `oboete hook --agent codex\|claude-or-grok --event <name>` | Agent-invoked entry: resolves `claude-or-grok` from `GROK_HOOK_EVENT`/`GROK_SESSION_ID`, checks the paused marker, drains stdin keeping 1 MB, runs the detector before any write, captures or spools (oversized payloads as `oversized` events), prints a plain-text pack on injection events | stdin JSON | pack or nothing; exit `0` |
| `oboete capture --agent pi --event <name> [--prior-failures <codes>]` | Pi's detached capture child; writes `spool/pi-ack/<event id>.started` first and renames it to `.done` on completion; records the extension's in-memory failure counters as diagnostics | stdin normalized event | none; exit `0` |
