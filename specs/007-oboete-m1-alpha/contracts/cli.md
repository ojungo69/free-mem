...
| `oboete hook --agent codex\|claude-or-grok --event <name>` | Agent-invoked entry: resolves `claude-or-grok` from `GROK_HOOK_EVENT`/`GROK_SESSION_ID`, checks the paused marker, reads at most 1 MB of stdin, runs the detector before any write, captures or spools (the read part of a larger payload is kept as a redacted `partial` row marked truncated, A7), prints a plain-text pack on injection events | stdin JSON | pack or nothing; exit `0` |
