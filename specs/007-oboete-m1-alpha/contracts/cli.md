...
| `oboete doctor [--probe-provider] [--no-probe-agents]` | Hook wiring by a fresh headless wiring probe per agent (default on, parallel, 90 s; `--no-probe-agents` shows the last result marked `unverified`, never `healthy`) and trust state; provider reachability by a live call only with `--probe-provider` (counts against the daily cap), otherwise the last outcome marked `unverified`; storage integrity and FTS5, migration level, worker liveness, spool backlog and writability, provider reachability, estimated allowance and exhaustion flag, catalog paid-plan flag, unrecognized agents, native memory coexistence, config file mode, Pi diagnostics. Every item has the shape `{ item, status, reason, consequence, recovery }`: `consequence` states what the developer loses while it is degraded (for example "Codex sessions capture nothing"), `recovery` states the command or step that repairs it (for corrupt storage: back up the file, run `oboete export` if readable, move the database aside, run `oboete setup`, import the export) | | table or JSON; exit `1` when any item is degraded, `3` on storage integrity failure |
| `oboete hook --agent codex\|claude-or-grok --event <name>` | Agent-invoked entry: resolves `claude-or-grok` from `GROK_HOOK_EVENT`/`GROK_SESSION_ID`, checks the paused marker, reads at most 1 MB of stdin, runs the detector before any write, captures or spools (a larger payload becomes a metadata-only `failed` row), prints a plain-text pack on injection events | stdin JSON | pack or nothing; exit `0` |
