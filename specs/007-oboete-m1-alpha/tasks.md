...
- [ ] T055 [EXT] [US1] Run the Claude Code ↔ Codex vertical slice under the isolated user (resume, compact, fork, clear behaviour; SessionStart source policy) and fix adapter mappings until the four Claude/Codex pairs pass (partial 2026-09-04: both Claude Code and Codex directions pass in the isolated run (docs/evidence/m1-dogfood.md); resume, compact, fork and clear are not exercised by the harness yet)
- [ ] T056 [EXT] [US1] Run the remaining pairs (Grok Build and Pi as sender and receiver) under the isolated user, including the Grok deferred delivery and the Pi child processes, until 12 of 12 pass; record in docs/evidence/m1-dogfood.md (blocked 2026-09-04: 6 of 12 pairs pass; every failure is a Grok Build leg refused with HTTP 402 because that account has no balance left. The Claude Code, Codex and Pi pairs pass in both directions)
