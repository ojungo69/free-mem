...
- [X] T014 [P] [EXT] Add the CI workflow matrix (Node 22.16 and 24.x) running typecheck, lint, build, test, pack-check in .github/workflows/ci.yml (done 2026-09-04: engine matrix 22.16.0 / 24.x + coverage lcov; sonar.sources still scripts (T088))
- [X] T015 [EXT] Measure real bundle cold start on 22.16 and 24.x and installed size in an empty prefix (R13 rows "cold start" and "installed size") and record in docs/evidence/m1-resource-envelope.md; block if over 100 ms or 30 MB (done 2026-09-04: docs/evidence/m1-resource-envelope.md + scripts/measure-cold-start.mjs; --version max 60 ms, hook max 252 ms, installed size 32.7 MB at 8e7a0fa then 29.15 MB once the bundled packages became devDependencies (both R13 rows pass))
