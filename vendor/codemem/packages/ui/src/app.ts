...
function startPolling() {
	...
	// doRefresh(), so it must run regardless of which tab is active.
...
function switchTab(tab: TabId, options: { canonicalHash?: boolean } = {}) {
...
function initTabs() {
	ALL_TAB_IDS.forEach((id) => {
		const btn = $(`tabBtn-${id}`);
		btn?.addEventListener("click", () => switchTab(id, { canonicalHash: true }));
	});

	// Listen for hash changes (back/forward navigation).
	window.addEventListener("hashchange", () => {
		const top = parseTabFromHash();
		if (top) switchTab(top);
	});
