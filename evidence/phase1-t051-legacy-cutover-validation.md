...
対象: `0be952c` + T051 working tree

## 結論

旧配置 DB の owner set を Linux `/proc/*/fd` と trusted absolute `lsof` の和集合で検査し、旧 owner が残る場合は canonical publish を開始しない。唯一の cutover daemon が WAL 上の `BEGIN EXCLUSIVE` を保持したまま read-only handle から online backup を生成・検証し、既存 storage journal で canonical artifact を publish する。

publish 後は setup ownership manifest を再検証し、旧 path を private tombstone directory への symlink へ atomic rename + parent fsync する。最後に recovery hardlink 経由で旧 inode の owner set が daemon だけであることを再検査し、その後だけ transaction/handle/control lock を解放する。

## closure

- `packages/core/src/legacy-cutover.ts`: 旧 process の bounded clean-stop request、2点 owner-set 検査、online backup/verify、journal publish、rollback hardlink、tombstone、split-brain fence。
...
- `P1-T051-03-tombstone-before-unlock`
