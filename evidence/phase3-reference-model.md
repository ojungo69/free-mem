...
各ゲートをわざと壊し、対応する test が落ちることを確認した。**148 件すべてで 1 件以上が失敗**し、
生存はゼロ、実行件数も期待どおり 148 件（黙って飛ばされた変異ゼロ）、復元後は 171/171 green。
