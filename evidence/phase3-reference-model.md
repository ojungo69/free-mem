...
各ゲートをわざと壊し、対応する test が落ちることを確認した。**199 件すべてで 1 件以上が失敗**し、
生存はゼロ、実行件数も期待どおり 199 件（黙って飛ばされた変異ゼロ）、復元後は 214/214 green
（`mutate.sh` が回すのは `reference-model.test.ts` 単体。`harness/continuity/*.test.ts` 全体は 324/324）。
...
| 順序違反で候補を巻き込む | 1 |
| 候補ゼロの terminal を台帳に入れる | 31 |
| 順序不明で候補を unknown にしない | 6 |
| start の取り込み連番を記録しない | 61 |
