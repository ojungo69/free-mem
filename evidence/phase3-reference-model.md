...
各ゲートをわざと壊し、対応する test が落ちることを確認した。**157 件すべてで 1 件以上が失敗**し、
生存はゼロ、実行件数も期待どおり 157 件（黙って飛ばされた変異ゼロ）、復元後は 178/178 green
...
| 兄弟から互換な候補を選ばない（derived id / native id 両方） | 22 |
| 再配送の相手を集合ごとに選ぶ | 7 |
