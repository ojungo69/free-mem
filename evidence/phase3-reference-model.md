...
各ゲートをわざと壊し、対応する test が落ちることを確認した。**187 件すべてで 1 件以上が失敗**し、
生存はゼロ、実行件数も期待どおり 187 件（黙って飛ばされた変異ゼロ）、復元後は 207/207 green
（`mutate.sh` が回すのは `reference-model.test.ts` 単体。`harness/continuity/*.test.ts` 全体は 315/315）。
