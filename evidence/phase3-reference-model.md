...
- **状態は権威として扱い、還元器は検証しない**（#35 / #43 で欄が増えたので明示する）。
  ...
  - **上限超えの復元配列を刈るのは記録経路だけ**。刈りは `recordDroppedEvidence` の中にあるので、
    ...
    反する状態を revision ごとに出し続けることになる）。刈りは 1 度で上限に収まるので、そこから
    先の再送は差分ゼロで収束する。**operation を閉じる terminal と放棄は記録経路を通らない**ので、
...
各ゲートをわざと壊し、対応する test が落ちることを確認した。**198 件すべてで 1 件以上が失敗**し、
生存はゼロ、実行件数も期待どおり 198 件（黙って飛ばされた変異ゼロ）、復元後は 213/213 green
（`mutate.sh` が回すのは `reference-model.test.ts` 単体。`harness/continuity/*.test.ts` 全体は 323/323）。
...
| 記録を末尾から落とす（FR-008） | 3 |
| 記録の上限検査を外す（FR-015） | 4 |
| 刈っただけの修復を捨てる（FR-015） | 1 |
| 記録の追加を別の診断で報告する（FR-009） | 12 |
| 記録の脱落を診断に出さない（FR-009） | 3 |
| 退避の記録で機密度を引き継がない | 1 |
| 孤児の記録を normal で残す | 3 |
| 孤児の記録を再送のたびに足す | 5 |
...
| 材料が欠けていても指紋の食い違いにする | 1 |
| 重複判定で配送鍵を見ず指紋だけにする（§8.2 の順を崩す） | 1 |
| 配送鍵の無い記録を同一性なしにする | 1 |
| 候補ゼロの terminal を状態に記録しない | 15 |
...
| 受理した terminal の指紋を残さない（FR-010） | 4 |
| unknown に倒した operation にも指紋を残す | 2 |
| 指紋の衝突検査を外す（FR-011） | 2 |
| 指紋が一致しても再配送として説明しない | 10 |
| 指紋を持たない旧い状態も衝突にする（FR-012） | 5 |
