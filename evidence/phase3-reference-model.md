...
各ゲートをわざと壊し、対応する test が落ちることを確認した。**77 件すべてで 1 件以上が失敗**し、
生存はゼロ、復元後は 101/101 green。

kill 率より先に**実行件数**を見ること。変異はソース中の文字列アンカーで当てるので、実装を直すと
`assert old in s` が落ちて `&&` が短絡し、その変異は**出力に何も出ないまま黙って飛ばされる**
（このラウンドだけで 9 件が外れた）。期待ラベル一覧と実行ラベル一覧の突き合わせ方はスクリプト
冒頭のコメントに書いてある。
