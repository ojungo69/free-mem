...
各ゲートをわざと壊し、対応する test が落ちることを確認した。**151 件すべてで 1 件以上が失敗**し、
生存はゼロ、実行件数も期待どおり 151 件（黙って飛ばされた変異ゼロ）、復元後は 175/175 green
...
| 順序不明で候補を unknown にしない | 9 |
| 退避で順序材料を刈らない | 2 |
| 同名が残るなら退避側の順序材料を残す | 1 |
| 再配送 start を nativeOperationId で拾わない | 11 |
...
| 兄弟から互換な候補を選ばない（derived id / native id 両方） | 21 |
| 再配送の相手を集合ごとに選ぶ | 6 |
| 兄弟の連結順を入れ替える | 1 |
| 飛ばした衝突兄弟を報告しない | 3 |
