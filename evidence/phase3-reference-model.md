...
各ゲートをわざと壊し、対応する test が落ちることを確認した。**150 件すべてで 1 件以上が失敗**し、
生存はゼロ、実行件数も期待どおり 150 件（黙って飛ばされた変異ゼロ）、復元後は 172/172 green。
...
| 順序不明で候補を unknown にしない | 8 |
| 退避で順序材料を刈らない | 2 |
| 同名が残るなら退避側の順序材料を残す | 1 |
| 再配送 start を nativeOperationId で拾わない | 10 |
| 再配送の判定を matchKey にする | 10 |
...
| native id を名乗る兄弟を優先しない | 1 |
| 届いた start が native id を持たなくても名乗る兄弟を優先する | 1 |
| 全件衝突のとき衝突の証拠を持たない | 9 |
...
| 再配送 start の原因 event を残さない | 2 |
| 放棄の配送衝突を診断に出さない | 1 |
| 同名 id でも側索引を引く | 3 |
| correlate の入口で terminal 相を要求しない | 1 |
| 兄弟から互換な候補を選ばない（derived id / native id 両方） | 4 |
| 再配送の相手を集合ごとに選ぶ | 3 |
| 兄弟の連結順を入れ替える | 1 |
| 飛ばした衝突兄弟を報告しない | 3 |
