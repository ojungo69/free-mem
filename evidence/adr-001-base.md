...
| fatal / non-fatal 分類完了 | **PASS** | `codemem/write-handle-classification.md`（T010: fatal 8 経路 + hook 補遺、いずれも局在・除去可能） |
| delta 比較記録 | **PASS** | `delta-comparison.md`（T011: vendor が 4 指標すべてで fork / greenfield に優位） |
| unsafe path 除去可能性 | **PASS（見込み）** | fatal 8 経路は observer-auth.ts / observer-client.ts dispatch / hook fallback ×4 / template ×1 に局在。除去後も BYOK HTTP 経路が独立残存することを dispatch 構造で確認。実除去と blocking 検証（runtime DB-open trace + static scan）は Phase 1 Exit |
...
3. **安全性の到達可能性**: sole-writer 違反と unsafe auth 経路はすべて特定可能・局在（T010）で、Phase 1 の物理削除計画（T012）が立つ。
...
- **greenfield（MIT 資産選択移植）**: gate 不合格時の予備案だった。gate 合格につき不発動。4026 件級のテスト資産をゼロから再構築するコストが決定的。

## 帰結

- `vendor/codemem/` に `.git` を含まない snapshot を取り込み（T014）、出所を `VENDOR.md` に記録。
- Phase 1 で T012 の A1–A7 を実施（fatal 8 除去 + carve-out 無効化）。Exit は Hard Invariant 4 の blocking 検証。
