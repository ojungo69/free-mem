...
mutate "    if (key === undefined || !recordedOrphans.has(key)) return true;" "    return true;" && run "孤児の記録を再送のたびに足す"
...
mutate "          : { eventId: pending.sourceEventIds[0] })," "          : {})," && run "退避の記録に兄弟を判別できる識別子を残さない"
...
mutate "        if (recorded.conflicted.length > 0) {" "        if (false) {" && run "同じ配送鍵の指紋食い違いを黙って重複にする"
mutate "    if (stored !== undefined && incoming !== undefined && stored !== incoming) conflicted.push(entry);" "    if (stored !== incoming) conflicted.push(entry);" && run "材料が欠けていても指紋の食い違いにする"
