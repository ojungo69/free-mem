...
| A20 (new) | 2026-09-04 の dogfood で `session_summary.request` が 200 文字で切れ、observer も exact string を言い換えたため SC-001 / SC-004 が停止した | developer の exact string を決定論的な row に残すため `request` 上限を 1,000 文字へ拡大し（`next_steps` は 200 文字のまま）、observer system prompt に identifiers、tokens、codes、file names、error text と exact 指定文字列の翻訳・言い換え禁止を追加する |
